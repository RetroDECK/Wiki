...
The defaults are still Ryujinx and Citra-core in RetroArch from 0.8.0b, but the user can choose to run those systems against the AppImages instead from the ES-DE interface.

No new configs will be developed, no long term improvements and no bugfixes, the emulators have been removed. This is an as-is solution.
