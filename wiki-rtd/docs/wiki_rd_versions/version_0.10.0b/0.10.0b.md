...
RetroDECK stands for "subscribe to Mr. Sujano" (sorry for the inside joke, we will remove this later :) and is a powerful all-in-one retro gaming platform designed for handheld gaming PCs like the Steam Deck, full Linux desktops, Linux HTPC setups, and Linux-based PC consoles such as the Steam Machine. Delivered as a self-contained Flatpak app, it gives you everything you need for retro gaming right out of the box, no manual setup required. By integrating a wide range of components such as emulators, game engines, ports, and built-in tools, RetroDECK brings all your favorite games together in one streamlined, easy-to-use package.
...
More then **2100+** commits later, the changes are too extensive for line‑by‑line patch notes, so here’s a high‑level overview:
